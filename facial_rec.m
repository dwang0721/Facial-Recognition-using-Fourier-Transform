...
% sample = imrotate(sample,90,'crop');
...
% output
strcat('best match in real: ', string(min_index_real))
strcat('best match in imarinary: ', string(min_index_imag))

%% take the real part and output a similar face
i = randi([1,10]);
predict_image_path = strcat('att_faces/s', string(min_index_real), '/', string(i), '.pgm');
output = double(imread(char(predict_image_path)))/255;
output = padarray(output,[8,18],'replicate');
